...
fn main() {
    ...
    while let Some(a) = args.next() {
        match a.as_str() {
            ...
            "--version" | "-V" => {
                println!("pragma-own {}", env!("CARGO_PKG_VERSION"));
                return;
            }
            "--runtime" | "-r" => {
    ...
    match pragma_own::check_paths_with(&paths, runtime) {
        Ok(result) => {
...
fn print_help() {
    println!(
        "pragma-own — static ownership/borrow checker for JS/TS\n\n\
         Usage:\n    pragma-own --check [--runtime node|bun|deno|none] <file-or-dir>...\n\n\
